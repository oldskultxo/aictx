...
def _continuity_quality_markdown(quality: dict[str, Any]) -> list[str]:
    ...
    for issue in issues[:3]:
        summary_text = str(issue.get("summary") or issue.get("code") or "Continuity quality issue").strip()
        recommendation = str(issue.get("recommendation") or "").strip()
        lines.append(f"- {summary_text}{(' ' + recommendation) if recommendation else ''}")
    return lines
