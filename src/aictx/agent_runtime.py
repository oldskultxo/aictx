...
5. Append the final AICTX summary to the final user response, using `finalized.agent_summary_text` as the canonical compact user-facing source; prefer structured render payloads when available and localize human-readable prose while preserving exact facts, technical tokens, and the details link/path.
...
- Do not enrich the final AICTX summary with invented facts; never invent data; only localize human-readable prose from structured factual fields when available and preserve file paths, commands, flags, test names, package names, code identifiers, and Markdown details links.
