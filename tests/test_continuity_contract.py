...
def _readme_artifact_lines() -> list[str]:
    ...
    marker = "The stable repo-local continuity artifact contract in `3.1.0` is:"
...
def test_pyproject_describes_continuity_runtime():
    ...
    assert payload["project"]["version"] == "3.1.0"
    assert payload["project"]["description"] == "Repo-local continuity runtime for coding agents"


def test_readme_contract_section_mentions_v31_and_required_global_codex_files():
    text = README_PATH.read_text(encoding="utf-8")

    assert "Current documented implementation: `3.1.0`" in text
