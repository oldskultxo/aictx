...
def _readme_artifact_lines() -> list[str]:
    ...
    marker = "The stable repo-local continuity artifact contract in `4.0.0` is:"
...
def test_pyproject_describes_continuity_runtime():
    ...
    assert payload["project"]["version"] == "4.0.0"
...
def test_readme_contract_section_mentions_v40_and_required_global_codex_files():
    ...
    assert "Current documented implementation: `4.0.0`" in text
