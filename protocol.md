...
- support multi-LLM agents through stable local `.ai_context_*` artifacts
- store only reusable information that saves future analysis time or avoids repeated failures

Query rules:
1. Read `.ai_context_memory/derived_boot_summary.json` when present.
2. Apply `.ai_context_memory/user_preferences.json` as defaults unless the current prompt overrides them.
