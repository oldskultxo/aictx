...
init-from-local:
	PYTHONPATH=src .venv/bin/python -m aictx init --repo . --yes
...
smoke: check-python $(VENV_READY) clean-smoke
	...
	$(AICTX_MODULE) internal execution prepare --repo .tmp/smoke-repo --task "review middleware behavior" --agent-id smoke-agent --execution-id smoke-prepare --files-opened src/a.py src/b.py --files-reopened src/a.py > .tmp/smoke-repo/prepared.json
...
wheel-install-check: check-python package-check
	WHEEL_PATH="$$( $(PYTHON) -c 'import pathlib,tomllib; print("dist/aictx-" + tomllib.loads(pathlib.Path("pyproject.toml").read_text(encoding="utf-8"))["project"]["version"] + "-py3-none-any.whl")' )"; \
	test -f "$$WHEEL_PATH"; \
	TMP_VENV="$$(mktemp -d)/aictx-wheel-venv"; \
