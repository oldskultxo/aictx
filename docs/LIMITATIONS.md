...
- that telemetry is measured-quality on every project (many repos will remain `insufficient_data` or `estimated` until sample and benchmark requirements are met)
...
- global metrics should be interpreted by `evidence_status` first (`insufficient_data | estimated | measured`), not by raw percentages alone
