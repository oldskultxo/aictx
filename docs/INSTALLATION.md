...
| Git-portable continuity | default `N` for new repos | Switches the AICTX-managed `.gitignore` policy, writes `.aictx/continuity/portability.json`, and in 6.5.0 can manage `.gitattributes` merge hints for the portable subset |
...
In `6.5.0`, `--portable-continuity` enables the team-safe profile for one engineer or small teams sharing the same Git repository. Git remains the transport; no external sync service is required.
